...
- 등록과 잠금 화면 인증이 하나의 프레임별 처리 파이프라인을 공유합니다. SCRFD 검출, 106개 랜드마크, 선택형 광도 정규화, 자세 게이트, 실재 얼굴 확인과 매칭이 동일한 프레임 상태를 사용합니다.
- MobileNetV2 머리 자세 모델을 추가했습니다. 잠금 화면 매칭은 허용된 자세만 사용하며 Yaw, Pitch, Roll 조정 방향을 번역된 안내로 표시합니다.
- 인증 세션, 파이프 읽기 스레드와 입력 스레드의 수명 관리를 재구성했습니다. 일반 잠금 해제에서는 마우스 이동을 무시하고 늦게 도착한 응답이 새 인증에 영향을 주지 않습니다.
- 광도 정규화는 기본적으로 꺼져 있습니다. 기존 저조도 강화 설정, 구형 노출 컨트롤러, 전역 하드웨어 오류 차단 목록과 진단 프로그램을 제거했습니다.
- 설치 프로그램에 Windows 기본 바탕 화면 바로 가기, 경량 독립 `Uninstall.exe`, 사용자 지정 확인 대화 상자, 설치 경로 검증과 동기화된 언어팩을 추가했습니다.

분류된 2.0.0 전체 변경 내역은 [CHANGELOG.md](CHANGELOG.md)에서 확인할 수 있습니다.
